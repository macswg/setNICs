            ...
            '''If (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator"))
{
    ...
    @staticmethod
    def create_toolbar_folder(folder_name: str) -> str:
        """
        Creates a new folder for the toolbar if it doesn't already exist.
        """
        command = [
            f'$toolbarPath = [System.IO.Path]::Combine([System.Environment]::GetFolderPath("MyDocuments"), "{folder_name}")',
            'if (-not (Test-Path $toolbarPath)) {New-Item -ItemType Directory -Path $toolbarPath}'
        ]
        return (command)
    ...
    def create_powershell_script(self):
        ...
        for nic in machine.nics:
            ...
            if self.dhcp_check(ip_addr) is False:  # If no DHCP
                nic_commands = [
                    ...
                    '$interface | New-NetIPAddress -IPAddress $IPAddress -PrefixLength $Prefix -DefaultGateway $Gateway'
                ...
                nic_commands = [
                    ...
                    'Set-DnsClientServerAddress -InterfaceAlias $InterfaceAlias -ResetServerAddress',
                    'Disable-NetAdapter -Name $InterfaceAlias -Confirm:$false',
                    'Start-Sleep -Seconds 5',  # Wait for the adapter to be fully disabled
        ...
        command_list.append(self.create_toolbar_folder(machine_name))
